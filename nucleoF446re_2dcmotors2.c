...
int main(void)
{
  ...
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_3); // Start PWM on TIM2 CH3
  ...
  while (1)
  {
    ...
    // Motor 1 control
    if (readValue1 < 1790) {
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET); // Set motor1 direction
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET);
      speed1 = (1790 - readValue1) / 3; // Calculate speed
    } else if (readValue1 > 2220) {
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET); // Set motor1 direction
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);
      speed1 = (readValue1 - 2220) / 3; // Calculate speed
    } else {
      speed1 = 0; // Stop motor if ADC value is in the middle range
    }
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, speed1); // Set PWM duty cycle for motor1

    // Motor 2 control
    if (readValue2 < 1790) {
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_RESET); // Set motor2 direction
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);
      speed2 = (1790 - readValue2) / 3; // Calculate speed
    } else if (readValue2 > 2220) {
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_RESET); // Set motor2 direction
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_SET);
      speed2 = (readValue2 - 2220) / 3; // Calculate speed
    } else {
      speed2 = 0; // Stop motor if ADC value is in the middle range
    }
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, speed2); // Set PWM duty cycle for motor2
